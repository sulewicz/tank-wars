...
CLANG_TIDY = "$(WASI_SDK_PATH)/bin/clang-tidy"
TIDY_FLAGS = --warnings-as-errors=* --config-file=.clang-tidy

# Whether to build for debugging instead of release
DEBUG = 0

# Compilation flags
CFLAGS = -W -Wall -Wextra -Werror -Wno-unused -MMD -MP -fno-exceptions -mbulk-memory
ifeq ($(DEBUG), 1)
	CFLAGS += -DDEBUG -O0 -g
else
...
.PHONY: lint
lint:
	$(CLANG_TIDY) $(wildcard src/*.c src/*.cpp) -- $(CFLAGS)
